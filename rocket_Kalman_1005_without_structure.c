...
float** Matrix_adder(float **matrix_1, float **matrix_2, int row, int col,
      int option);
float** Matrix_multiplicator(float **matrix_1, float **matrix_2, int r1, int c1,
      int r2, int c2);
void KF_alg(float **F, float **H, float **R, float **esti_state_prev,
      float **esti_cov_prev, float **measu_state, float sigma, float del_t,
	  float **esti_state, float **esti_cov);
scalar_KF_return scalar_KF_alg(float F_scalar, float H_scalar, float R_scalar,
      float esti_state_prev, float esti_cov_prev, float measu_state,
      float sigma_w, float del_t);
...
//      printf("test while\n"); ok
      ...
      average_pos_z[iter_temp][0][0] = average_filter(tmp_values)
            - standard_alt;
      ...
      //for x pitch ; rho
      printf("test111\n"); // problem.
      float sigma_w_rho_x = measu_Ang_acc_x[iter_temp][0][0]
            - measu_Ang_acc_x[iter_temp - 1][0][0];

      KF_alg(F_vec_gyro, H_vec_gyro, R_mat_gyro,
            est_state_rho_x[iter_temp - 1], est_cov_rho_x[iter_temp - 1],
            measu_Ang_acc_x[iter_temp], sigma_w_rho_x, del_t,
			est_state_rho_x[iter_temp],est_cov_rho_x[iter_temp]);

      /*
      est_state_rho_x[iter_temp][0][0] =
            KF_rho_x_vel_ang.estimate_state[0][0];
      est_state_rho_x[iter_temp][1][0] =
            KF_rho_x_vel_ang.estimate_state[1][0];
       * */
      printf("test\n"); // problem.
      ...
      //for y yaw ; pi
      float sigma_w_pi_y = measu_Ang_acc_y[iter_temp][0][0]
            - measu_Ang_acc_y[iter_temp - 1][0][0];
      KF_alg(F_vec_gyro, H_vec_gyro, R_mat_gyro,
            est_state_pi_y[iter_temp - 1], est_cov_pi_y[iter_temp - 1],
            measu_Ang_acc_y[iter_temp], sigma_w_pi_y, del_t,
			est_state_pi_y[iter_temp],est_cov_pi_y[iter_temp]);
      /*
      ...
      float sigma_w_thea_z = measu_Ang_acc_z[iter_temp][0][0]
            - measu_Ang_acc_z[iter_temp - 1][0][0];
      KF_alg(F_vec_gyro, H_vec_gyro, R_mat_gyro,
            est_state_thea_z[iter_temp - 1], est_cov_thea_z[iter_temp - 1],
            measu_Ang_acc_z[iter_temp], sigma_w_thea_z, del_t,
			est_state_thea_z[iter_temp],est_cov_thea_z[iter_temp]);
      /*
      est_state_thea_z[iter_temp][0][0] =
            KF_thea_z_vel_ang.estimate_state[0][0];
      est_state_thea_z[iter_temp][1][0] =
            KF_thea_z_vel_ang.estimate_state[1][0];
       * */
      ...
      if (est_state_thea_z[iter_temp][1][0] > (math_pi / 4)) {
         ...
         lambda = (float) atan(
               tan((double) est_state_rho_x[iter_temp][1][0])
                     / sin((double) est_state_thea_z[iter_temp][1][0]));
      } else {
         //calc with cosine  /use pi and thea
         lambda = (float) atan(
               tan((double) est_state_pi_y[iter_temp][1][0])
                     / cos((double) est_state_thea_z[iter_temp][1][0]));
      }

      //with lambda, we should calculate gravity comp of each axis.
      //gravity = 9.81m/s^2
      gravity_x = gravity_const * sin((double) lambda)
            * cos((double) est_state_thea_z[iter_temp][1][0]);
      gravity_y = gravity_const * sin((double) lambda)
            * sin((double) est_state_thea_z[iter_temp][1][0]);
      ...
      //for x
      float sigma_w_x = measu_acc_x[iter_temp][0][0]
            - measu_acc_x[iter_temp - 1][0][0];
      KF_alg(F_vec, H_vec, R_mat, est_state_x[iter_temp - 1],
            est_cov_x[iter_temp - 1], measu_acc_x[iter_temp], sigma_w_x,
            del_t, est_state_x[iter_temp],est_cov_x[iter_temp]);
       ...
       * */
      est_state_x[iter_temp][2][0] = est_state_x[iter_temp - 1][2][0]
            + est_state_x[iter_temp - 1][1][0] * del_t
            + est_state_x[iter_temp - 1][0][0] * del_t * del_t / 2;

      //for y
      float sigma_w_y = measu_acc_y[iter_temp][0][0]
            - measu_acc_y[iter_temp - 1][0][0];
      KF_alg(F_vec, H_vec, R_mat, est_state_y[iter_temp - 1],
            est_cov_y[iter_temp - 1], measu_acc_y[iter_temp], sigma_w_y,
            del_t, est_state_y[iter_temp],est_cov_y[iter_temp]);
       ...
       * */
      est_state_y[iter_temp][2][0] = est_state_y[iter_temp - 1][2][0]
            + est_state_y[iter_temp - 1][1][0] * del_t
            + est_state_y[iter_temp - 1][0][0] * del_t * del_t / 2;

      //for z
      scalar_KF_return KF_z_pos;

      //여기서 동적할당이 필요하려나??!
      float sigma_w_z_acc = measu_acc_z[iter_temp][0][0]
            - measu_acc_z[iter_temp - 1][0][0];
      KF_alg(F_vec, H_vec, R_mat, est_state_z[iter_temp - 1],
            est_cov_z[iter_temp - 1], measu_acc_z[iter_temp], sigma_w_z_acc,
            del_t, est_state_z[iter_temp],est_cov_z[iter_temp]);
      ...
      float sigam_w_pos = average_pos_z[iter_temp][0][0]
            - average_pos_z[iter_temp - 1][0][0];
      KF_z_pos = scalar_KF_alg(F_scalar, H_scalar, R_scalar,
            est_state_z[iter_temp - 1][2][0],
            est_cov_z_for_pos[iter_temp - 1],
            average_pos_z[iter_temp][0][0], sigam_w_pos, del_t);
      ...
      printf("[ Pos_X : %.2f ]\n[ Pos_Y : %.2f ]\n[ Pos_Z : %.2f ]\n\n",
            est_state_x[iter_temp][2][0], est_state_y[iter_temp][2][0],
            est_state_z[iter_temp][2][0]);
      printf("[ Ang_X : %.2f ]\n[ Ang_Y : %.2f ]\n[ Ang_Z : %.2f ]\n\n",
            est_state_rho_x[iter_temp][1][0],
            est_state_pi_y[iter_temp][1][0],
            est_state_thea_z[iter_temp][1][0]);
...
//    HAL_Delay(50);
      ...
      if (Parachute == 0)  // set 1
         ...
         accelAngleX = atan2f(myAccelScaled.y,
               sqrtf(
                     myAccelScaled.x * myAccelScaled.x
                           + myAccelScaled.z * myAccelScaled.z)); //RAD
...
//      printf("AngleX: %.2f AngleY: %.2f\n",AngleX,AngleY);
         ...
         /*printf("Rocket Angle: %.2f\r\n", Rocket_Angle); // test code
          HAL_Delay(500);*/
         //  ????  ?   ??? ?  ?  ?   ??? 방향?   ?  ?   ?  ?  ?  켜서 중력  ????  ?   ?   ???
         accZ_raw = myAccelScaled.y * cos(AngleX)
               - myAccelScaled.z * sin(AngleX); // z y
         accY_raw = myAccelScaled.z * cos(AngleX)
               + myAccelScaled.y * sin(AngleX); // y z
               //  ????  ?   ??? ?  ?  ?   ??? 방향?   ?  ?   ?  ?  ?  켜서 중력  ????  ?   ?   ???
      ...
      if (tmp_count == iter-1) { //when arrives in 1000, escape while
         break;
      }
...
float** Matrix_adder(float **matrix_1, float **matrix_2, int row, int col,
      int option)  //option == 1 : adder / == 0 : substracter
{
   if (option == 1) {
      for (int i = 0; i < row; i++) {
         for (int j = 0; j < col; j++) {
...
void KF_alg(float **F, float **H, float **R, float **esti_state_prev,
      float **esti_cov_prev, float **measu_state, float sigma_w, float del_t,
	  float **estimate_state, float **estimate_cov) {

   //float **estimate_cov = Matrix_gen_2dim(row_2, col_2);
   //KF.estimate_state = Matrix_gen_2dim(row_2, col_1);
   float **Kalman_Gain = Matrix_gen_2dim(row_2, col_1);
   ...
   //predict state
   predict_state = Matrix_multiplicator(F, esti_state_prev, row_2, col_2,
         row_2, col_1);

   //predict cov
   float sigma_sq = sigma_w * sigma_w;
   //set Q
   Q[0][0] = del_t * del_t * sigma_sq;
   Q[0][1] = del_t * sigma_sq;
   Q[1][0] = del_t * sigma_sq;
   Q[1][1] = sigma_sq;

   predict_cov = Matrix_multiplicator(F, esti_cov_prev, row_2, col_2, row_2,
         col_2);
   transp_F = Transpose(F, row_2, col_2);
   predict_cov = Matrix_multiplicator(predict_cov, transp_F, row_2, col_2,
         row_2, col_2);
   predict_cov = Matrix_adder(predict_cov, Q, row_2, col_2, add);

   //Kalman Gain
   inv_term_1 = Matrix_multiplicator(H, predict_cov, row_1, col_2, row_2,
         col_2);
   inv_term_2 = Matrix_multiplicator(inv_term_1, transp_H, row_1, col_2, row_2,
         col_1);
   Kalman_Gain = Matrix_multiplicator(predict_cov, transp_H, row_2, col_2,
         row_2, col_1);
   **inv_term_tot = 1 / (**inv_term_2);

   Kalman_Gain = Matrix_multiplicator(Kalman_Gain, inv_term_tot, row_2,
         col_1, row_1, col_1);

   //Estimation of state
   innov_1 = Matrix_multiplicator(H, predict_state, row_1, col_2, row_2,
         col_1);
   innov_tot = Matrix_adder(measu_state, innov_1, row_1, col_1, substract);
   estimate_state = Matrix_multiplicator(Kalman_Gain, innov_tot, row_2,
         col_1, row_1, col_1);
   estimate_state = Matrix_adder(predict_state, estimate_state, row_2,
         col_1, add);

   //Estimation of cov.
   renewal_cov_1 = Matrix_multiplicator(Kalman_Gain, H, row_2, col_1, row_1,
         col_2);
   renewal_cov_1 = Matrix_multiplicator(renewal_cov_1, predict_cov, row_2,
         col_2, row_2, col_2);
   estimate_cov = Matrix_adder(predict_cov, renewal_cov_1, row_2, col_2,
         substract);
   ...
   Matrix_free_2dim(Kalman_Gain, row_2);
